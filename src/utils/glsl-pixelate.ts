...
const createFallbackPixelateCanvas = (
  ...
  if (context) {
    ...
    if (tempCtx) {
      ...
      // Scale back up without smoothing for pixelated effect
...
export const createPixelatedCloneCanvas = (
  image: HTMLImageElement,
  width: number,
  height: number,
  ...
  if (!targetWidth || !targetHeight) {
    return createFallbackPixelateCanvas(image, 1, 1, pixelSize);
  }
